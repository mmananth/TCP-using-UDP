...
struct
{
	...
	unsigned int payload_size[WINDOW];
...
int send_ack()
{
	...
	{
		/*START: Make sure only data that is received is sent to ftps*/
		cb.payload_size[(pkt->hdr.th_seq)%WINDOW] = (bytes_recv_troll - sizeof(struct sockaddr_in)
				- sizeof(struct tcphdr));
		/*END: Make sure only data that is received is sent to ftps*/
		...
		while(cb.exists[cb.expected_ack%WINDOW] == TRUE)
		{
			...
			if(sendto(TCPDM1_out_socket, cb.pack[cb.expected_ack%WINDOW].payload, cb.payload_size[cb.expected_ack%WINDOW], 0,
			...
			printf("sent seq %u %u size data from tcpd_m1 to %s \n",cb.expected_ack,
					cb.payload_size[cb.expected_ack%WINDOW],inet_ntoa(TCPDM1_out_addr.sin_addr));
			cbDel();
			cb.expected_ack++;
		}
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		if (FD_ISSET(TCPDM1_in_socket, &TCPDM1_in_set))
		{
			...
			bzero(&troll_msg, sizeof(troll_msg));
