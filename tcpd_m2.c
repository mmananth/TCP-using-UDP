...
void calculate_RTO(unsigned long int M)
{
	...
	//printf("The calculated RTO is %u\n", RTO);
}
	...
	if(wrap_up_mode >= FIN_WAIT2 && ((hdr->th_flags & TH_FIN )!=0))
	{
		...
		if(cbIsEmpty()==TRUE)
		{
			if(send_finack(hdr)==FAILED)
				return FAILED;
			printf("Buffer empty and fin received snding ack entering time wait\n");
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		if (FD_ISSET(TCPDM2_in_socket, &TCPDM2_in_set)&&cbIsFull()!=TRUE)
		{
		...
		else if (FD_ISSET(TCPDM2_out_socket, &TCPDM2_in_set))
		{
